...
.. module:: bug_as
   :platform: Unix
   :synopsis: Algorithm for robot navigation

.. moduleauthor:: Prof. Carmine Recchiuto

This node for bug0 algorithm for robot navigation. The robot follows a desired point using the go-to-point behavior and switches to wall following behavior when it encounters obstacles. It uses laser scan data to detect obstacles and determine its state. The bug0 algorithm is encapsulated in an action server, allowing external goal setting and preempting. The code also includes necessary callbacks, services, and publishers/subscribers for communication and control.
...
Clients:
    /go_to_point_switch /wall_follower_switch
